...
public class MovementController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		try {
	        ...
	        if (action.equalsIgnoreCase("insertMovement")) {				
	        	...
	        	createMovementEditPage(req, resp);
	...
	private void createMovementEditPage(HttpServletRequest req, HttpServletResponse resp) {
		try {
			PrintWriter out = resp.getWriter();			
			int id = Integer.parseInt(req.getParameter("id"));
			Movement mov = movementService.getMovementById(req, id);
			boolean isEntry = mov.getType().equals("entry");
			...
			out.print("<html>");
				...
				out.print("<body>");
					out.print("<div id=main>");
						out.print("<form action=MovementController?action=saveMovement method=POST>");
							out.print("<div class=container>");
								out.print("<h3>Movement</h3>");
								out.print("<label><b>Id</b></label>");
								out.print("<input type=number id=inputReadonly placeholder=Enter id name=id value=" + mov.getId() + " readonly=readonly>");
								out.print("<label><b>Product</b></label>");
								out.print("<br>");
								out.print("<select id=productName name=productName disabled=disabled>");
									out.print("<option value=" + mov.getProductName() + ">" + mov.getProductName() + "</option>");
								out.print("</select>");
								out.print("<br>");
								out.print("<br>");
								out.print("<label><b>Type</b></label>");
								out.print("<br>");
								out.print("<select id=type name=type> value=" + mov.getType() + ">");
									if (isEntry) {
										out.print("<option value=entry selected>Entry</option>");
										out.print("<option value=exit>Exit</option>");
									} else {
										out.print("<option value=entry>Entry</option>");
										out.print("<option value=exit selected>Exit</option>");										
									}
								out.print("</select>");
								out.print("<br>");
								out.print("<br>");
								out.print("<label><b>Quantity</b></label>");
								out.print("<input type=number placeholder=Enter quantity name=quantity id=quantity value=" + mov.getQuantity() + " required>");
								out.print("<br>");
								out.print("<button type=submit class=registerbtn>Register</button>");
							out.print("</div>");
						out.print("</form>");
			...
			System.out.println("createMovementEditPage error: " + ex.getMessage());
		}
	}
}
